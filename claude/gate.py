#!/usr/bin/env python3
"""SHAPE gate for Claude Code — PreToolUse hook enforcing phases, budget, and rules.
...
Usage in .claude/settings.json:
  "hooks": {"PreToolUse": [{"matcher": "*", "command": "python3 /path/to/gate.py"}]}
"""
...
RULES_FILE = Path(os.environ.get("SHAPE_RULES", os.path.expanduser("~/.claude/shape/rules.shape")))
DEFAULT_BUDGET = float(os.environ.get("SHAPE_BUDGET", "5.0"))

WRITE_TOOLS = {"write", "edit", "bash", "mcp__*", "computer"}
READ_TOOLS = {"read", "glob", "grep", "web_search", "mcp__read*"}
TOOL_COSTS = {"bash": 0.05, "write": 0.02, "edit": 0.02, "computer": 0.10}

PHASE_ALLOWS_WRITE = {"explore": False, "decide": False, "commit": True}

def tool_effect(name):
    if name in {"read", "glob", "grep", "web_search"} or name.startswith("mcp__read"):
        return "read"
    if name in {"write", "edit", "bash", "computer"} or name.startswith("mcp__write"):
        return "write"
